...
호스트 : 네트워크에 연결된 장치에게 부여되는 고유한 이름이며 일반적으로 IP 주소(ex: 172.217.175.35:443) 대신 이해하기 쉬운 호스트 이름(ex: https://www.google.co.kr) 으로 사용된다.
...
웹 페이지의 프로토콜, 호스트, 포트를 확인 하는 방법은 간단하다. 개발자 도구를 열고 location을 입력하면 Web API인 Location 객체에 접근해 확인할 수 있다.
...
4. 브라우저는 자신이 보낸 요청의 Origin과 서버가 보낸 응답의 Access-Control-Allow-Origin를 비교한 후
   유효한 응답인지 아닌지를 결정한다.
...
![Preflight Request](https://github.com/chanyDev/TIL/blob/main/img/Web/preflight.png?raw=true)
...
실제 브라우저가 보낸 요청을 확인하면, 단순히 Origin에 대한 정보뿐 아니라 예비 요청 이후에 보낼 실제 요청에 대한 다른 정보들도 포함되어 있는것을 확인할 수 있다.

예비 요청에서 브라우저는 Access-Control-Request-Headers를 사용하여 실제 요청에서 content-type 헤더를 사용할 것을 알려주거나, Access-Control-Request-Method를 사용하여 GET 메서드를 사용할 것을 서버에게 미리 알려준다.
...
![Simple Request](https://github.com/chanyDev/TIL/blob/main/img/Web/simple.png?raw=true)
...
대부분의 HTTP API는 text/xml 이나 application/json과 같은Content-type을 갖도록 설계되기 때문에
...
```js
fetch('https://chany-dev.tistory.com/15', {
  credential: 'inclue', // 모든 요청에 인증 정보 포함
});
```

[axios](https://axios-http.com/)의 경우 withCredentials 옵션을 true로 설정한다.

```js
axios.get('https://chany-dev.tistory.com/15', { withCredentials: true });
```
...
2. 응답 헤더에는 Access-Control-Allow-Credentials: true를 지정하여 자격 증명으로 실제 요청을 수행할 수 있음을 나타내야 한다.

![Credential Request](https://github.com/chanyDev/TIL/blob/main/img/Web/credential.png?raw=true)

<br>

## CORS 해결 방법

CORS 정책 위반으로 에러가 발생했을때 해결하는 방법에 대해서도 알아보자.

### Access-Control-Allow-Origin 응답 헤더 설정

가장 대표적인 해결 방법은 Access-Control-Allow-Origin 헤더에 알맞은 값을 설정해 주는것이다.

와일드카드인 \*을 사용하면 모든 요청에 대해 리소스를 허용할 수 있지만, 아무런 출처에나 요청을 허용할 경우 보안에 문제가 발생할 수 있으니 특정 출처에만 요청 가능하도록 명시적으로 설정하는것이 좋다.
...
다음과 같이 cors 모듈을 사용하여 권한을 설정할 수도 있다.

```js
// cors 모듈을 사용하여 특정 출처 접근 허용
...
webpack-dev-server의 [proxy](https://webpack.js.org/configuration/dev-server/#devserverproxy) 기능을 사용하면 간단하게 CORS 정책을 우회할 수 있다.

백엔드 서버가 http://localhost:3000 에서 동작하고 있다고 가정했을 때 아래와 같이 설정할 수 있다.
...
위와 같이 설정하게 될 경우 로컬 환경에서 /api 로 시작하는 URL로 보내는 요청에 대해 웹팩이 http://localhost:3000 으로 요청을 프록싱 해주기 때문에 CORS 정책을 우회할 수 있다.

설정 X : http://localhost:8000 (클라이언트) => http://localhost:3000 (서버) CORS 위반<br>
설정 O : http://localhost:8000 (클라이언트) => 프록시 설정된 Dev Server => http://localhost:3000 (서버) CORS 우회

### package.json 설정

CRA로 생성한 프로젝트에서는 package.json 파일에 proxy값을 설정하여 proxy 기능을 활성화 할 수 있다.
